...
class PhotoContentPresenter implements IPhotoContent.Presenter {
        ...
        Observable
                ...
                .subscribe(new Consumer<Boolean>() {
                    ...
                    public void accept(@NonNull Throwable throwable) throws Exception {
                        view.onShowNetError();
                        ErrorActionUtils.print(throwable);
                    }
                });
    }
    ...
    private Boolean saveImage(String url) {
        ...
        try {
            // 获取 bitmap
            Bitmap bitmap = Glide.with(PPUtils.getAppContext()).load(url).asBitmap()
            ...
            if (bitmap != null) {
                ...
                // 其次把文件插入到系统图库
//                MediaStore.Images.Media.insertImage(PPUtils.getAppContext().getContentResolver(), file.getAbsolutePath(), fileName, null);
                // 最后通知图库更新
                PPUtils.getAppContext().sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse("file://" + path)));
